# Ask Then Build (Clarify & Build Protocol)
...
`ask-then-build` provides a streamlined, zero-overhead alignment loop:
...
```bash
mkdir -p ~/.gemini/config/skills/ask-then-build
cp SKILL.md ~/.gemini/config/skills/ask-then-build/SKILL.md
```

### Project-Specific Skill
Place `SKILL.md` under your workspace `.agents/skills/` directory:
```bash
mkdir -p .agents/skills/ask-then-build
cp SKILL.md .agents/skills/ask-then-build/SKILL.md
```
